...
The Vantiq projects contained in this repo take the information contained in the Kamelet definitions, constructing
Vantiq assemblies that contain services and Vantiq sources ready to use in a Vantiq system.

## Terminology and Naming

Generally, there are two assemblies provided for each system for which assemblies are created. Assemblies for 
...
Camel version 3.21.x for consuming information from Amazon's AWS S3 will be in the assembly

    com.vantiq.extsrc.camel.kamelets.v3_21_0.aws_s3_source

while an assembly for sending data to Amazon's AWS SNS will be in assembly
...
The assemblies contained herein are composed of the following components.
...
* Vantiq Source -- this is the Vantiq component that represents the external system to the Vantiq environment.
  ...
  * The Vantiq source is the in the package matching the assembly name with the name of the kamelet as the suffix 
    `_source`.
* Vantiq Service -- this is the Vantiq component with which Vail code interacts.
...
* Vantiq Documents -- there are a few documents that may be present in the assembly.
  ...
  * Route Template Document -- `<assembly name>_routes.yaml`.  This is the routing information used by the Camel 
    ...
    * Notes Document (optional) -- `<assembly name>_notes.md`.  When present, this contains information that Vail 
      developers need to know about use of the assembly. Typically, this is non-standard information regarding message 
...
# Assembly Use in a Vantiq Namespace

## Assembly Installation

To make use of any assembly defined herein, the user should perform the following tasks.
...
* From that catalog, import the assembly or assemblies desired, providing the configuration properties as 
  requested. 
  * These configuration properties
  will often include addressing information and credentials for the remote systems. It is strongly suggested that 
    credential information (or other confidential information) should be stored in Vantiq 
    Secrets, and referenced using the `@secrets()` notation.

## Using the Assemblies

Once installed, each assembly creates a source & service for interacting with that source.
...
described above.  This imported assembly is a prerequisite for the other Camel Kamelet-based assemblies.


**Note**: Some assemblies (specifically those that work with systems with very limited messaging capabilities),
...
Users of these systems will need to be aware of the_impendance mismatch_ and respond accordingly.


# Logging & Debugging

These assemblies themselves provide no direct support for logging or debugging -- they are simply specifications for 
