...
public class HikVisionHandleConfiguration extends Handler<ExtensionServiceMessage> {
    ...
    /**
     * Interprets the configuration message sent by the Vantiq server and sets up the HikVision Source.
     */
    @Override
    public void handleMessage(ExtensionServiceMessage message) {
        ...
        {
            cameraList.add(new CameraEntry(cameras.get(i)));
        }
      ...
      /*
        ...
        */
        ...
        if ( !(config.get(DRV_IMAGE_FOLDER_PATH) instanceof String )) {
                log.warn("Configuration doesn't include {} . User default value {}",DRV_IMAGE_FOLDER_PATH,dvrImageFolderPath);
        ...
        if ( !(config.get(VANTIQ_DOCUMENT_PATH) instanceof String )) {
                log.warn("Configuration doesn't include {} . User default value {}",VANTIQ_DOCUMENT_PATH,vantiqDocumentPath);
        ...
        if ( !(config.get(VANTIQ_RESOURCE_PATH) instanceof String )) {
                log.warn("Configuration doesn't include {} . User default value {}",VANTIQ_RESOURCE_PATH,vantiqResourcePath);
    ...
    /**
     * Closes the source {@link HikVisionCore} and marks the configuration as completed. The source will
