 ...
 * configuration. All sources must belong to
 * the same VANTIQ Server &amp; Namespace (i.e. same authtoken). Options are in
 * a file where each option has its own line,
 * and is in the form {@code property=value} with no quotes required for
 * Strings. Options are:
 * <ul>
 ...
 * you wish to connect. Whitespace
...
public class FTPClientMain {
    ...
    static final Logger log = LoggerFactory.getLogger(FTPClientMain.class);
    static List<FTPClientCore> sources;
     ...
     */
    private static void startSources(List<FTPClientCore> sources) {
        for (FTPClientCore source : sources) {
     ...
     */
    static List<FTPClientCore> createSources(Properties config) {
        ...
        for (String sourceName : sourceNames) {
            ...
            source = new FTPClientCore(sourceName, authToken, targetVantiqServer);
            sources.add(source);
        }
