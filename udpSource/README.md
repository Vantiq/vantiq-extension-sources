...
### UDP Options
*	defaultListeningPort -- Sets the default port to which sources will bind if no other port is specified. Defaults to 3141 when not set
*	defaultListeningAddress -- Sets the default address to which the sources will bind if no other address is specified. Attempts to find a valid address when not set.
...
* 	passCsvOutFrom: Optional. A string specifying the location of an array of objects that will be converted into CSV format. Requires useCsvSchema to be set. Default is null
...
* receiveServers: Optional. An array of pairs that specify both an address and port to receive UDP messages from. A pair is formatted as an array containing first the address as a string containing either the URL or IP address, and second the port number. If left empty, defers to the other address and port settings
