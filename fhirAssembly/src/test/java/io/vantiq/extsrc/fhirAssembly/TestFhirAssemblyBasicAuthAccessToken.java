...
public class TestFhirAssemblyBasicAuthAccessToken extends TestFhirAssembly {
    ...
    public static String TEST_ACCESS_TOKEN = "IAmATestAccessTokenForAServerThatDoesn'tCare";
    ...
    public static Map<String,?> getAssemblyConfigForTest() {
        Map<String, ?> config =  Map.of(
                "basicAccessToken", TEST_ACCESS_TOKEN,
    ...
    @Test
    public void test000ValidateSourceBasicToken() throws Exception {
        ...
        assertTrue(FHIR_SOURCE + ": wrong username", Strings.isNullOrEmpty(config.get("username").toString()));
        assertTrue(FHIR_SOURCE + ": wrong password", Strings.isNullOrEmpty(config.get("password").toString()));
        assertEquals(FHIR_SOURCE + ": wrong password", TEST_ACCESS_TOKEN, config.get("accessToken"));
